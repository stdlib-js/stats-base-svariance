...
var svariance = tryRequire( resolve( __dirname, './../lib/svariance.native.js' ) );
var opts = {
...
tape( 'the function has an arity of 4', opts, function test( t ) {
	t.strictEqual( svariance.length, 4, 'has expected arity' );
	t.end();
});
...
tape( 'if provided a `correction` parameter yielding `N-correction` less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = svariance( x.length, x.length, x, 1 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = svariance( x.length, x.length+1, x, 1 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
